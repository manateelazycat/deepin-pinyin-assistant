...
struct speech_rec_notifier {
	...
	void (*speechEndHandler)(int reason);	/* 0 if VAD.  others, error : see E_SR_xxx and msp_errors.h  */
};
...
int sr_init(struct speech_rec * sr, const char * sessionBeginParams, enum sr_audsrc aud_src, struct speech_rec_notifier * notifier);
