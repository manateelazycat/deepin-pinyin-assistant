...
static void end_sr_on_error(struct speech_rec *sr, int errcode)
{
	...
	if (sr->session_id) {
		if (sr->notif.speechEndHandler)
			sr->notif.speechEndHandler(errcode);
...
static void end_sr_on_vad(struct speech_rec *sr)
{
	...
	while(sr->rec_stat != MSP_REC_STATUS_COMPLETE ){
		...
		if (rslt && sr->notif.resultHandler)
			sr->notif.resultHandler(rslt, sr->rec_stat == MSP_REC_STATUS_COMPLETE ? 1 : 0);
	...
	if (sr->session_id) {
		if (sr->notif.speechEndHandler)
			sr->notif.speechEndHandler(END_REASON_VAD_DETECT);
...
int sr_init_ex(struct speech_rec * sr, const char * sessionBeginParams, 
				...
				struct speech_rec_notifier * notify)
{
	...
	if (sessionBeginParams == NULL) {
		sessionBeginParams = DEFAULT_SESSION_PARA;
	}
	...
	param_size = strlen(sessionBeginParams) + 1;
	sr->sessionBeginParams = (char*)SR_MALLOC(param_size);
	if (sr->sessionBeginParams == NULL) {
		sr_dbg("mem alloc failed\n");
		return -E_SR_NOMEM;
	}
	strncpy(sr->sessionBeginParams, sessionBeginParams, param_size);
	...
	if (aud_src == SR_MIC) {
		...
		update_format_from_sessionparam(sessionBeginParams, &wavfmt);
...
fail:
	...
	if (sr->sessionBeginParams) {
		SR_MFREE(sr->sessionBeginParams);
		sr->sessionBeginParams = NULL;
	}
...
int sr_init(struct speech_rec * sr, const char * sessionBeginParams, 
		enum sr_audsrc aud_src, struct speech_rec_notifier * notify)
{
	return sr_init_ex(sr, sessionBeginParams, aud_src, 
...
int sr_start_listening(struct speech_rec *sr)
{
	...
	session_id = QISRSessionBegin(NULL, sr->sessionBeginParams, &errcode); //听写不需要语法，第一个参数为NULL
	...
	if (sr->notif.speechBeginHandler)
		sr->notif.speechBeginHandler();
...
int sr_stop_listening(struct speech_rec *sr)
{
	...
	while (sr->rec_stat != MSP_REC_STATUS_COMPLETE) {
		...
		if (NULL != rslt && sr->notif.resultHandler)
			sr->notif.resultHandler(rslt, sr->rec_stat == MSP_REC_STATUS_COMPLETE ? 1 : 0);
		Sleep(100);
	}
